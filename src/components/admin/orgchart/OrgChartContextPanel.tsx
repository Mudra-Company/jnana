...
const SkillChip: React.FC<{
  ...
  const missing =
    'border border-dashed border-red-300 dark:border-red-700/60 text-red-500 dark:text-red-300/80 bg-transparent';
  const neutral =
    tone === 'hard'
...
const SkillsComparison: React.FC<{ role: CompanyRole; assignee?: User | null }> = ({
  role,
  assignee,
}) => {
  const hasAssignee = !!assignee && isRealPerson(assignee);
  const userHardList = hasAssignee ? (assignee!.hardSkills || []) : [];
  const userHardNames = userHardList.map(s => s.name);
  const userSoftNames = hasAssignee ? (assignee!.karmaData?.softSkills || []) : [];

  const reqHard = role.requiredHardSkills || [];
  const reqSoft = role.requiredSoftSkills || [];

  const hardMatched = reqHard.filter(s => hasAssignee && fuzzySkillMatch(s.name, userHardNames)).length;
  const softMatched = reqSoft.filter(s => hasAssignee && fuzzySkillMatch(s.name, userSoftNames)).length;
  ...
  const counterTone = (matched: number, total: number) => {
    ...
    if (matched === total) return 'text-green-600 dark:text-green-400';
    if (matched === 0) return 'text-red-600 dark:text-red-400';
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-[10px] text-gray-400 dark:text-gray-500">
        ...
        {!hasAssignee && (
          <span className="ml-auto italic">Posizione aperta — nessun assegnatario</span>
        )}
      </div>
      ...
      {reqHard.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {reqHard.map(s => {
              const owned = hasAssignee && fuzzySkillMatch(s.name, userHardNames);
              ...
              return (
                <SkillChip
                  ...
                  name={s.name}
                  tone="hard"
                  state={!hasAssignee ? 'neutral' : owned ? 'owned' : 'missing'}
      ...
      {reqSoft.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {reqSoft.map(s => {
              const owned = hasAssignee && fuzzySkillMatch(s.name, userSoftNames);
              return (
                <SkillChip
                  key={s.name}
                  name={s.name}
      ...
      {hasAssignee && (extraHard.length > 0 || extraSoft.length > 0) && (
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">
            Altre skill di {assignee!.firstName}
          </h4>
          <div className="flex flex-wrap gap-1">
            {extraHard.map(s => (
              <span
                key={`eh-${s.name}`}
                className="text-xs px-2 py-0.5 rounded-full border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300"
